...
public class UserServiceImpl extends BaseService implements UserService {
	...
	public User userLogin(UserVo userVo) {
		System.out.println("密码" + userVo.getUserPassword());
		System.out.println("账号" + userVo.getUserAccount());
		// MD5加密
	...
	public void addUserInfo(UserVo userVo) {
		...
		if (null != userVo.getUserMark() && userVo.getUserMark().equals(MessageConstant.USER_TYPE_SUPER + "")) {
			// 添加的用户信息为系统管理员
			userVo.setUserType(MessageConstant.USER_TYPE_SUPER);
		} else {
			// 添加的用户信息为普通用户
			userVo.setUserType(MessageConstant.USER_TYPE_NORMAL);
		}
		...
		// 给添加的用户分配角色
		if (null != userVo.getUserMark() && userVo.getUserMark().equals(MessageConstant.USER_TYPE_SUPER + "")) {
			this.userDao.insertUserHasRole(userVo.getUserId(),MessageConstant.USER_TYPE_SUPER);
		} else {
			this.userDao.insertUserHasRole(userVo.getUserId(),MessageConstant.USER_TYPE_NORMAL);
		}
